> **番外 · 论文原版（v6 终稿）**
>
> 本篇是《Claude Science 绿皮书》第 09 篇《疟疾红细胞分类：换了三次看的方式，召回率从 84% 爬到 96%》背后的完整论文原版。正文一字未改；仅将原写作工具留下的图片占位标记接回本仓库 `articles/assets/day09/` 下的真实配图，图注、表格、公式、参考文献均保留原文。
>
> - 绿皮书第 09 篇正文：[`articles/day09-preview.md`](../articles/day09-preview.md)
> - 配图目录：`articles/assets/day09/`
> - 数据与代码：私有素材库 `claude-science-materials` 的 `day09/`
>
> *(English note: full original research paper behind Green Book chapter 09. Text verbatim; only the broken image-placeholder tokens were rewired to the real figures in `articles/assets/day09/`.)*
...
![图1 感染与未感染红细胞样本示例](../articles/assets/day09/fig01_sample_cells.png)

*图 1 感染与未感染红细胞样本示例。上排为感染细胞（可见内部紫红色疟原虫结构），下排为未感染细胞。*

![图2 图像尺寸分布](../articles/assets/day09/fig02_class_size_dist.png)
...
![图3 SVM混淆矩阵](../articles/assets/day09/fig03_svm_confusion.png)

*图 3 SVM 模型在测试集上的混淆矩阵。左下角为假阴性 118 例（漏检感染），右上角为假阳性 58 例。*

简单 CNN 将假阴性从 118 降至 34，感染召回率提升至 94.33%。这表明卷积层的局部感受野能有效捕捉疟原虫斑点等局部空间模式。

![图4 CNN训练曲线](../articles/assets/day09/fig05_cnn_training_curves.png)

*图 4 简单 CNN 的训练与验证准确率/损失曲线（12 个 epoch）。验证准确率在 epoch 8 左右趋于稳定，最高约 96.08%。*

![图5 CNN混淆矩阵](../articles/assets/day09/fig06_cnn_confusion.png)

*图 5 简单 CNN 在测试集上的混淆矩阵。假阴性降至 34 例，假阳性 25 例。*

![图6 SVM与CNN性能对比](../articles/assets/day09/fig08_model_comparison.png)
...
![图7 预训练迁移学习模型性能对比](../articles/assets/day09/fig09_advanced_transfer_comparison.png)

*图 7 SVM、简单 CNN、EfficientNet-B0 冻结特征和微调模型的性能对比。左图为准确率/召回率/F1 分数，右图对比假阴性和假阳性数量。*

![图8 EfficientNet-B0微调混淆矩阵](../articles/assets/day09/fig10_efficientnet_finetuned_confusion.png)
